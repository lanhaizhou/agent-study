 ...
 * 将故事片段向量化存入内存向量库，根据用户问题做语义检索，再把检索结果作为上下文交给大模型回答。
 * 演示「文档 → 向量 → 相似度检索 → 拼进 prompt → 生成回答」的完整流程。
 */
...
// 嵌入模型：把文本转成向量，用于构建向量库和查询时的相似度计算（需与向量库使用同一模型）
...
// 原始文档：每段故事内容 + 元数据（章节、角色、类型、心情等），后续会向量化并存入向量库
const documents = [
  new Document({
...
// 内存向量库：用 embeddings 把 documents 转成向量并存入内存；适合小规模数据，生产环境可换为 Pinecone、Chroma 等持久化向量库
const vectorStore = await MemoryVectorStore.fromDocuments(documents, embeddings);

// 检索器：封装「按问题检索最相关的 k 条文档」的逻辑，k=3 表示每次最多取 3 个片段
...
for (const question of questions) {
  ...
  // 用问题做语义检索：retriever 内部会对 question 做向量化，再与库中向量做相似度计算，返回最相关的 k 条 Document
  const retrievedDocs = await retriever.invoke(question);

  // 带相似度评分的检索（用于打印）：返回 [Document, score][]，score 为距离，越小越相似；相似度可理解为 1 - score
  const scoredResults = await vectorStore.similaritySearchWithScore(question, 3);

  // 打印本次检索到的文档及其相似度，便于理解 RAG 选中的内容
  console.log("\n【检索到的文档及相似度评分】");
  ...
  // 把检索到的文档拼成「故事片段」上下文，插入到 prompt 中，供大模型基于这些片段回答问题
