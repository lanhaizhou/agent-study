/**
 * all-tools.mjs
 * 为 LangChain 提供的可绑定到 Agent 的通用工具集：
 * 读文件、写文件、执行系统命令、列出目录。供 LLM 在对话中按需调用。
 */
...
// ---------------------------------------------------------------------------
// 3. 执行命令工具（带实时输出）
// 使用 spawn + stdio: "inherit" 将子进程输出直接打到当前控制台。
// ---------------------------------------------------------------------------
const executeCommandTool = tool(
  async ({ command, workingDirectory }) => {
    ...
    // 子进程异步结束，用 Promise 包装以便工具返回结果给 Agent
