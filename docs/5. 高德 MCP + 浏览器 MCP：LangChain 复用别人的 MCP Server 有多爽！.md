...
对应代码：在 `chapter/5/tool-test` 的 LangChain MCP 调用基础上，增加多 MCP Server 配置（如 `mcp-test.mjs`）。环境变量：根目录 `.env` 中除 `OPENAI_*` 外，按需配置 `AMAP_MAPS_API_KEY`（高德）、`ALLOWED_PATHS`（FileSystem MCP 可访问目录，逗号分隔）。
...
- **环境变量**：高德需 `AMAP_MAPS_API_KEY`；FileSystem 需 `ALLOWED_PATHS`（逗号分隔的可访问路径）；其余与第 4 章一致（OPENAI\_\*、dotenv 等）。
